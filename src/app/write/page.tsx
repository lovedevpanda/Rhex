...
export default async function WritePage({ searchParams }: WritePageProps) {
  ...
  const canEditThisPost = Boolean(editingPost && (editingPost.authorId === user.id || user.role === "ADMIN" || user.role === "MODERATOR"))
  const isStillEditable = Boolean(editingPost?.editableUntil && new Date(editingPost.editableUntil).getTime() > Date.now()) || user.role === "ADMIN" || user.role === "MODERATOR"
